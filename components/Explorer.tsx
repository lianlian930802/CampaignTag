    ...
    <div className="space-y-6 relative min-h-[700px]">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-400">精细化打标体系：支持多维标签分类与飞书同步</p>
        </div>
          ...
          <div className="flex items-center gap-2">
            {selectedIds.length > 0 && (
              <button 
                ...
                className="px-4 py-2.5 bg-slate-50 text-slate-500 hover:bg-slate-100 rounded-xl text-sm font-bold transition-all border border-slate-200"
              >
                取消选择
              </button>
            )}
                      ...
                      <div className="space-y-0.5">
                        {cat.tags.map(tag => (
                          ...
                          >
                            ...
                            {filterTags.includes(tag.label) ? (
                              <Check className="w-3.5 h-3.5 text-indigo-600" />
                            ) : (
                              <div className="w-3.5 h-3.5 rounded border border-slate-200 group-hover:border-slate-300"></div>
                            )}
                          </button>
                        ))}
                      </div>
                ...
                <td className="px-6 py-5">
                  <div className="flex flex-wrap gap-2">
                    {campaign.tags.length > 0 ? campaign.tags.map(tagLabel => (
                      <span 
                        key={tagLabel} 
                        className="inline-flex items-center px-2.5 py-0.5 bg-indigo-50 text-indigo-600 rounded-lg border border-indigo-100 text-[11px] font-bold whitespace-nowrap"
                      >
                        {tagLabel}
                      </span>
      ...
      {/* COMPACT BULK TAG MODAL (No AI button, footer button renamed to '绑定') */}
      {showBulkTagModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 w-full max-w-6xl flex overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh]">
            ...
            {/* Right Main Content (L2 Focused Grid) */}
            <div className="flex-1 flex flex-col bg-white">
              ...
              {/* Grid - Refined to 6 Columns for small items, with light borders preserved */}
              <div className="flex-1 overflow-y-auto p-8 bg-slate-50/20 custom-scrollbar">
                <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                  {activeCategory?.tags.filter(t => t.label.toLowerCase().includes(modalSearchL2.toLowerCase())).map(tag => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isActive && (
                          <div className="absolute top-2.5 right-2.5 p-0.5 bg-indigo-600 rounded-full text-white scale-75">
                  ...
                  {activeCategory?.tags.length === 0 && (
                    <div className="col-span-full py-20 text-center text-slate-300 italic">该维度下暂无可用标签</div>
                  )}
                </div>
              </div>
            ...
            <button onClick={() => setShowBulkTagModal(false)} className="absolute top-8 right-8 p-2 hover:bg-slate-100 rounded-xl text-slate-400 transition-all z-[110]">
              <X className="w-6 h-6" />
            </button>
          </div>
        </div>
